...
from process import train, test, worker
from optimize import calcF1
...
def work(mode, train_data, test_data, dev_data, model, args, sampleround, epoch):
    for e in range(epoch):
        ...
        for b in range(batchcnt):
            ...
            acc, cnt, tot = train(b, model, data, sampleround, mode,
                                  dataQueue, resultQueue, freeProcess,
                                  lock, args.numprocess)
...
if __name__ == "__main__":
    ...
    model = Model(args.lr, args.dim, args.state_dim, args.hidden_dim, dm.eventcnt, wv)
    model.cuda()
    if args.start != '':
    ...
    if args.test == True:
        ...
        for b in range(batchcnt):
            data = test_data[b * args.batchsize_test : (b+1) * args.batchsize_test]
            acc_, cnt_, tot_ = test(b, model, data, ["EE", "AD"],
        ...
        work(["EE", "AD", "pretrain"], train_data, test_data, dev_data, model, args, 1, args.epochPRE)
    else:
        work(["EE", "AD"], train_data, test_data, dev_data, model, args, args.sampleround, args.epochRL)
